# File Search Engine
This application is a local file search engine that uses threading and a queue to manage IO tasks in order to keep the gui interactive. The `Treeview` updates the results in real-time and sets the focus and view on the most recently inserted result in the results `Treeview`.
